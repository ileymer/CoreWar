...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/20 09:02:43 by dgeonosi          #+#    #+#             */
...
void	ld_helper(t_cursor *curs, t_dataall *inf, int type_arg[4], int arg[4])
{
	if (type_arg[1] == 2)
		arg[1] = (unsigned int)get_value(inf, curs->pc + 2,
...
void	run_ld(t_cursor *curs, t_dataall *inf, int cycles)
{
	...
	{
		ld_helper(curs, inf, type_arg, arg);
	}
